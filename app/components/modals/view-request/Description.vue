...
const requestStore = useViewRequestFormData();
...
// Watch for store changes and update editor content
watch(
  () => requestStore.description,
  (newDescription) => {
    if (editor.value && newDescription !== editor.value.innerHTML) {
      editor.value.innerHTML = newDescription || '';
      const content = editor.value.innerText.trim();
      isEmpty.value = content === '';
    }
  },
...
onMounted(() => {
  ...
  // Initialize editor with store content
  if (editor.value) {
    editor.value.innerHTML = requestStore.description || '';
    const content = editor.value.innerText.trim();
    isEmpty.value = content === '';
  }
});
...
<template>
  <div
    :class="`editor-container mt-1 opacity-75 ${
  ...
  >
    ...
    <div class="editor-wrapper relative w-full h-[12rem]">
      ...
      <div
        v-show="isEmpty"
        v-else-if="status.toLowerCase() === 'pending'"
        class="placeholder-overlay"
        aria-hidden="true"
      >
        {{ placeholderText }}
