...
watch(
  () => formData.purpose,
  (newPurpose) => {
    selectedPurpose.value = newPurpose;
  }
);
...
<template>
  <div ref="containerRef" :class="`relative flex select-none  ${token.user_id !== userId || status.toLowerCase() !== 'pending' ? 'pointer-events-none' : ''}`">
    <button
      type="button"
      @click="setShow(!show)"
      class="border max-w-[14rem] select-none truncate grid grid-cols-[auto_1fr] relative border-[#CFCFCF] leading-[normal] font-p3 font-medium p-1 pr-2 space-x-1 items-center rounded-[0.3rem] transition-all duration-150 hover:bg-gray-50 border-[#CFCFCF"
    >
