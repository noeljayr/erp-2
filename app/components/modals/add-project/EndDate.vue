...
function selectDate(day: Date): void {
  ...
  if (selectedDate.value.toString().length > 0) {
    formData.setEndDate(selectedDate.value.toISOString());
    setShow(false);
  } else {
    formData.setEndDate(null);
  }
}
...
<template>
  <div ref="containerRef" class="relative flex select-none">
    ...
    >
      <IconCalendarCheck class="h-4 w-4 mr-1" />
      <span class="leading-[normal]">{{
        formData.endDate && formData.endDate.length > 0
          ? formatDate3(formData.endDate)
          : "End date"
      }}</span>
    </button>
    ...
    <AnimatePresence>
      ...
      >
        <span
          ...
          >End project scheduled to end (optional)</span
        >
